...
-- 1. Полная очистка прошлых сессий скрипта
...
local function initPlayerESP(player)
    ...
    local function setup()
        ...
        -- Создаем 8 линий для уголков бокса
        local cornerLines = {}
        for i = 1, 8 do
...
-- 4. Блокировка метатаблиц (Защита от тряски CFrame)
...
mt.__newindex = newcclosure(function(t, k, v)
    if t == Camera and (k == "CFrame" or k == "CoordinateFrame" or k == "Focus") then
        if isScriptActive and isRmbPressed and currentTargetPart then
            return 
        end
    end
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if isScriptActive and s and p.Character and not isAlly(p) then
            ...
            if r and h and h.Health > 0 then
                ...
                if onScreen then
                    -- Метрики размеров (Коэффициенты увеличены для полного охвата модельки)
                    local sc = 1 / (sPos.Z * 2) * 1000
                    local w, hDim = 3.0 * sc, 5.2 * sc -- Размеры бокса увеличены
                    local x, y = sPos.X - w / 2, sPos.Y - hDim / 2
                    local cornerLength = w / 4 
                    
                    -- Отрисовка Уголков (Corners)
                    -- Верхний левый угол
                    ...
                    -- Полоска здоровья (отодвинута чуть дальше влево из-за увеличения бокса)
